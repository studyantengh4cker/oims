...
export function GraduatingStudentForm({
	...
	const [selectedCourse, setSelectedCourse] = useState<string | null>(defaultValues?.course || null);
	...
	const onSubmit = async (values: GraduatingStudentFormData) => {
		try {
			...
			if (defaultValues) {
				...
				await createStudent(values.studentId,
                    values.firstName,
                    values.middleName || "",
                    values.lastName,
                    values.email,
                    ...
                    values.yearLevel,
                    values.classification);
				router.push(`/graduation/records`);
			}
			toast({
